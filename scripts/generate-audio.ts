...
async function main() {
  ...
  for (const selected of scenarios) {
    ...
    console.log(`Preparing ${selected.title}`);
    ...
    // Publish only after all scenario clips are saved. No credentials are included.
    const manifest = { provider: 'ElevenLabs', voiceId, modelId, clips };
    const manifestPath = join(process.cwd(), 'public', selected.manifest);
