...
import { Detection, formatTime, riskLevel, safetyAction, scoreRisk, tactics } from '@/lib/scenario';
...
export default function Dashboard() {
  ...
  return choosing ? <ScenarioPicker initialId={selected.id} onSelect={next => { setSelected(next); setChoosing(false); }} /> : <ScenarioDashboard key={selected.id} selected={selected} onChoose={() => setChoosing(true)} />;
}

function ScenarioDashboard({ selected, onChoose }: { selected: Scenario; onChoose: () => void }) {
  const scenario = selected.lines;
  const call = useCall(selected);
  const { status, elapsed, lineCount } = call;
  const [showInfo, setShowInfo] = useState(false);
  const [mode, setMode] = useState<AnalysisMode>('mock');
  const [session, setSession] = useState(0);
  const transcriptRef = useRef<HTMLDivElement>(null);
  const analysis = useNemotron(lineCount, elapsed, mode, session, selected.id);
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell" id="dashboard">
      ...
      <main>
        ...
        <div className="scenario-bar"><div className="scenario-icon"><Fingerprint size={22} /></div><div><span className="tiny-label">THE SCENARIO</span><strong>{selected.title}</strong><p>{selected.description}</p></div><span className="scenario-tag">{selected.difficulty}</span><button className="audio-refresh" onClick={onChoose}>Change scenario</button></div>
        ...
        <div className="dashboard-grid">
          <section className="panel call-panel">
            ...
            <div className="caller-row"><div className="caller-avatar"><Phone size={23} /><span><ArrowDownLeft size={10} /></span></div><div className="caller-details"><strong>{selected.caller}</strong><span>Unverified caller <i /> Synthetic scenario</span></div><div className="call-time"><span>{formatTime(elapsed)}</span><small>CALL DURATION</small></div></div>
          ...
          <section className={`panel risk-panel ${tone}`}>
            ...
            <div className="risk-caption"><span className="risk-pill"><span />{displayRisk}</span><p>{mode === 'nemotron' && analysis.error ? 'Only previously validated signals are shown.' : analysisPending ? 'Waiting for validated model results.' : score >= 75 ? 'Multiple tactics. One clear warning.' : score >= 50 ? 'A sensitive request changes the picture.' : score >= 25 ? 'Pressure is building. Stay alert.' : score > 0 ? 'A warning sign deserves attention.' : 'Listening for the first warning sign.'}</p></div>
        ...
        <section className={`action-panel ${tone}`} aria-live="polite"><div className="action-icon">{score >= 50 ? <Shield size={24} /> : <ShieldCheck size={24} />}</div><div><span className="tiny-label">{score >= 50 ? 'RECOMMENDED SAFETY ACTION' : 'STAY ONE STEP AHEAD'}</span><h3>{score >= 50 ? 'Protect your information and money.' : score >= 25 ? 'Pause the pressure. Verify the story.' : 'Trust is earned, not announced.'}</h3><p>{safetyAction(detections, selected.verification)}</p></div><span className="action-badge">{score >= 50 ? 'ACT WITH CAUTION' : 'SAFETY FIRST'}</span></section>

        <section className="panel timeline-panel" id="timeline"><div className="panel-heading"><h2><Activity size={17} /> Risk timeline</h2><span className="muted">{mode === 'mock' ? 'The moments that matter' : 'Time detected · may lag the transcript'}</span></div><div className="timeline">{events.length ? events.map((event, i) => <div className={`timeline-event ${event.score >= 50 ? 'critical' : ''}`} key={i}><div className="timeline-top"><span className="timeline-dot" /><time>{formatTime(event.at)}</time><strong>{event.score}<small> / 100</small></strong></div><h4>{event.detections.map(d => tactics[d.type].label).join(' + ')}</h4><p>+{event.score - (events[i - 1]?.score ?? 0)} risk points</p><blockquote>“{event.detections.map(d => d.evidence).join(' … ')}”</blockquote></div>) : <div className="timeline-empty"><Clock3 size={18} /><span>Every increase has a reason. Start a call to trace the evidence.</span><span>{call.voice ? 'Audio-synchronized' : `00:00 — ${formatTime(selected.duration)}`}</span></div>}</div></section>
        {status === 'ended' && <section className="summary panel"><div><FileText size={22} /><h2>Call summary</h2><span>{analysis.error ? 'Analysis incomplete' : analysisPending ? 'Finishing analysis…' : !call.completed ? 'Ended early' : 'Simulation complete'}</span></div><p>In {elapsed} seconds, this simulation surfaced <strong>{detections.length} {detections.length === 1 ? 'tactic' : 'tactics'}</strong> and reached <strong>{score}/100 risk</strong>. {detections.some(d => d.type === 'otp_request') ? 'The turning point was the request to share a verification code.' : 'Review the evidence above to see which signals appeared.'}</p><p className="muted">{mode === 'mock' ? 'These are scripted detections, not a judgment about a real caller.' : 'These are model classifications of a synthetic call, not proof of fraud. The score includes validated tactics accumulated so far.'} A low score does not establish that a call is safe.</p><button className="primary-button" onClick={onChoose}>Try Another Scenario<ArrowRight size={16} /></button></section>}
